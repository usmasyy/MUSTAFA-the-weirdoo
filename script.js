...
function setupLizard(size, legs, tail) {
  ...
  var initialDelay = 50; // 3 seconds (30 frames per second)

  setInterval(function() {
    ctx.clearRect(0, 0, canvas.width, canvas.height);

    if (initialDelay > 0) {
      // Draw the "Farah the weird monster" on the black screen for the first 3 seconds
      ctx.fillStyle = "black";
      ctx.fillRect(0, 0, canvas.width, canvas.height);

      ctx.font = `${critter.textSize}px Arial`;
      ctx.fillStyle = critter.textColor;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText("mustafa the weird monster",canvas.width / 2, canvas.height / 2);

      initialDelay--;
      if (initialDelay === 0) {
        // After 3 seconds, start the actual animation
        critter.follow(Input.mouse.x, Input.mouse.y);
      }
    } else {
      // Continue the regular animation
      critter.follow(Input.mouse.x, Input.mouse.y);
    }
  }, 33);
}
